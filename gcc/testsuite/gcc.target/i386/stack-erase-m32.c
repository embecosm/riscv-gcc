...
int test(int (*func) (int, int))
{
  ...
  // arg 1 and 2 ($250 and $240) are pushed onto the stack but they won't
  // get erased as the caller is responsible for them and is non-stack erase.

  // Without -fcombine-stack-adjustments (-O0), the args are deallocated
  // when the callee returns.

  // With -fcombine-stack-adjustments (-O1 or higher) these args are not
  // deallocated from the stack until the caller returns. initial_sp is
  // adjusted above to reflect this, otherwise sp is restored to the wrong
  // position after by the zero test.
  func(250, 240);
