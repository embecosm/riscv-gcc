...
int f1 (int x, int y)
{
  int a = f1_sub (x, y); /* { dg-error "Cannot call non-stack-erase function 'int f1_sub.int, int.' from stack-erase function" } */
  return a + 4;
}
...
{
  int a = myfunc(x, y); /* { dg-error "cannot call using function pointer .myfunc. from stack.erase function" } */
  return a + 5;
}
...
int f2 (int x, int y)
{
  int a = f2_sub (f1_sub, x ,y);
  return a + 6;
}

__attribute__((stack_erase))
int f3 (int x, int y); /* { dg-error "'stack_erase' attribute present on 'int f3.int, int.'" } */
...
__attribute__((stack_erase))
int f4 (int x, int y) /* { dg-error "'stack_erase' attribute present on 'int f4.int, int.'" } */
{
  return x + y;
}
